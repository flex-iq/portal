...
export const WorkoutCreate: React.FC = () => {
    ...
    const handleSetChange = (exerciseId: number, setId: number, field: keyof Set, value: number) => {
    ...
    return (
        <Box>
            ...
            {/* Client Create  */}
            <ClientCreate addExercise={() => addExercise()} />
            {/* Workout Create  */}
            {workouts[0]?.id > 0 &&
                <Grid container spacing={2} sx={Styles.Grid.Container}>
                    <Grid>
                        <Button variant="outlined" size="large" onClick={() => addExercise()} >
                            Create Exercise
                        </Button>
                    </Grid>
                </Grid>
                ...
                <Box sx={{ height: '500px', overflow: "auto", border: '2px solid #ccc', borderRadius: '8px' }}>
                    {exercises.map((exercise) => (
                        <Box key={exercise.id} style={{ border: "1px solid #ccc", padding: "20px", marginBottom: "20px", borderRadius: "8px" }}>
                            <IconButton aria-label="delete" color="error" onClick={() => removeExercise(exercise.id)}>
                                ...
                                <Grid container key={set.id} spacing={Styles.Grid.Spacing} sx={Styles.Grid.Container}>
                                    <Grid>
                                        <TextField
                                            ...
                                            // value={set.reps}
                                            value={undefined}
                                            onChange={(e) => handleSetChange(exercise.id, set.id, "reps", Number(e.target.value))}
                                        />
                                    </Grid>
                                    <Grid>
                                        <TextField
                                            ...
                                            type="number"
                                            value={undefined}
                                            onChange={(e) => handleSetChange(exercise.id, set.id, "resistance", Number(e.target.value))}
                                        />
                                    </Grid>
                                    <Grid>
                                        <TextField
                                            ...
                                            type="number"
                                            value={undefined}
                                            onChange={(e) => handleSetChange(exercise.id, set.id, "tempo", Number(e.target.value))}
                                        />
                                    </Grid>
                                    <Grid>
                                        <TextField
                                            ...
                                            type="number"
                                            value={undefined}
                                            onChange={(e) => handleSetChange(exercise.id, set.id, "rpe", Number(e.target.value))}
                                        />
                                    </Grid>
                                    ...
                                    <Grid>
                                        <Stack direction="row" spacing={Styles.Grid.Spacing} alignItems="center">
                                            ...
                                            <IconButton aria-label="delete" color="error" onClick={() => removeSet(exercise.id, set.id)}>
                                                {/* <DeleteIcon /> */}
                                                <CloseIcon fontSize="large" />
                                            </IconButton>
                                        </Stack>
                            ...
                            <Button variant="contained" color="primary" onClick={() => addSet(exercise.id)}>
                                Add Set
                            </Button>
                        </Box>
                    ))
